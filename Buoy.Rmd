...
# 1. Abstract
This report seeks to use 20-years of data NDBC Station 44013 to check whether there is global warming. After data collection and cleaning, time series model and linear regression model were used to fit the data. Conclusions were drawn based on the results of models.

# 2. Data Collection
Data was abstracted from the NDBC website. 30-years of data was collected from 1987 to 2016. Six variables were selected namely, year(YY), month(MM), day(DD), hour(hh), air temperature(ATMP) and sea surface temperature(WTMP). In order to see how the trend of temperature change, the only two related variables ATMP and WTMP were selected.
```{r include = FALSE}
...
# 4. Model Fitting
Data was first plotted to see if any model could be considered. The time series model was considered given the shape of the plot. Also, the linear regression model was considered since it was a very simple model and the sign of the slope can be used to check the trend of climate change.
...
# 5. Conclusion
From the time series model, both predicted dropping and rising trend of temperature were shown. For the linear regression model, a strong rising trend of air temperature was shown. Based on the data of air temperature, a sign of global warming was indicated. Based on the data of sea surface temperature, the sign of global warming is not that intense. However, no conclusion can be drawn regarding whether global warming caused the rising of air temperature in this area. The time series model used in this report is rather simple. A more complex model is likely to give a more accurate prediction. 
